...
async function main() {
  ...
  class Limite extends React.Component {
    ...
    componentDidCatch(e) { erros.push('CRASH: ' + e.message); }
  ...
  const rotulo = (el) => (el.textContent || '').trim().slice(0, 40).replace(/\s+/g, ' ');
  ...
  const irAba = async (nome) => {
    for (let g = 0; g < 3; g++) {
      const v = alvos('button').find(x => /^(Voltar|Cancelar)/.test(x.textContent.trim()));
      if (!v) break;
      await clique(v);
    }
  ...
  for (const nome of abas) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!el) continue;
      const via = nome + ' > ' + rotulo(el);
      const antes = erros.length;
      try { await clique(el); } catch (e) { erros.push('THROW: ' + e.message); }
      for (let k = antes; k < erros.length; k++) erros[k] += '  <<< ' + via;
      // subtela: clica em cada botao uma vez, voltando ao ponto
      const m = Math.min(alvos(SEL_SUB).length, 25);
      for (let j = 0; j < m; j++) {
        const e2 = alvos(SEL_SUB)[j];
        if (!e2) continue;
        const via2 = via + ' >> ' + rotulo(e2);
  ...
  console.log('\n=== cliques: ' + cliques + ' | ERROS (' + erros.length + ') ===');
  console.log([...new Set(erros)].join('\n'));
}
main().catch(e => { console.error = origErr; console.log('FALHA HARNESS', e); });
