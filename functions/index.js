...
Você também pode DESENHAR para ensinar: se um esquema ajudar muito a entender (mapa de aplicação das massas, mamelões e efeitos incisais, curva de queima, anatomia do dente), ou se o aluno pedir um desenho/imagem/esquema, termine a resposta com UMA linha exatamente neste formato: [ILUSTRAR: detailed English description of the diagram]. Use no máximo uma ilustração por resposta e apenas quando agregar de verdade — a explicação em texto continua completa mesmo sem o desenho.
ESTILO dos desenhos de ESTRATIFICAÇÃO/camadas (o aluno exige assim): igual aos mapas de estratificação de protético — o dente é SEMPRE um INCISIVO CENTRAL SUPERIOR (dente anterior, formato de pá, SEM cúspides, SEM raízes múltiplas — NUNCA molar/pré-molar, a não ser que o aluno pergunte sobre dente posterior), desenhado em VISTA VESTIBULAR (de frente), contorno simples, fundo branco, as zonas de cada massa pintadas em aquarela suave (mamelões subindo do terço cervical, halo incisal no bordo), e o NOME/CÓDIGO de cada massa escrito ao lado com linha fina apontando a zona. Use os códigos REAIS dos materiais da biblioteca do aluno quando existirem. NÃO desenhe corte anatômico/transversal. No [ILUSTRAR: ...] escreva em inglês e comece obrigatoriamente com "upper central incisor, labial view, flat shovel-shaped anterior tooth crown" e inclua os códigos como labels.
Para CURVA DE QUEIMA: gráfico simples de linha (temperatura × tempo) com os valores do material do aluno marcados.
    ...
    for (const modelo of MODELOS) {
      ...
      if (texto) {
        ...
        if (mIl) {
          ...
          if (podeIlustrar) {
            const promptImg = `Clean, professional dental ceramics educational diagram for a dental technician learning feldspathic porcelain, white background, no watermark. If it depicts layering/stratification: draw in the style of a dental lab layering chart — an UPPER CENTRAL INCISOR (anterior tooth, flat shovel-shaped crown, labial/front view — never a molar or premolar, no cusps, no visible roots), simple outline with soft watercolor-painted layer zones (mamelons rising from the cervical third, incisal halo along the edge) and small text labels with thin pointer lines naming each ceramic mass code; NOT an anatomical cross-section. ${mIl[1].trim()}`;
            const MODELOS_IMG = ['gemini-2.5-flash-image', 'gemini-3.1-flash-image-preview', 'gemini-3-pro-image-preview'];
            for (const mImg of MODELOS_IMG) {
              try {
                const ri = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${mImg}:generateContent?key=` + chave, {
