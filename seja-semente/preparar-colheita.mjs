// Prepara a COLHEITA nas nuvens da Apple e do Google, num tiro só:
//   1. Registra o app iOS com.sejasemente.colheita no Firebase e imprime o
//      GoogleService-Info.plist (base64) para commitar
//   2. Na Apple: registra o bundle ID, liga a permissão de PUSH e gera o
//      perfil de distribuição "Colheita AppStore" (base64 no log)
...
// O aplicativo de quem investiu — prestação de contas em sorrisos
const BUNDLE = 'com.sejasemente.colheita';
...
console.log('══ 1. Firebase (app iOS da Colheita) ══');
let registro = ((await gapi('GET', `/projects/${PROJETO}/iosApps`)).json.apps || []).find(a => a.bundleId === BUNDLE);
if (!registro) {
  const cria = await gapi('POST', `/projects/${PROJETO}/iosApps`, { bundleId: BUNDLE, displayName: 'Colheita (iOS)' });
...
if (cfg.json.configFileContents) {
  console.log('── PLIST nativo-colheita ──');
...
if (bid) console.log(`✓ Bundle ID já registrado (${bid.id})`);
  ...
  const cria = await api('POST', '/bundleIds', {
    data: { type: 'bundleIds', attributes: { identifier: BUNDLE, name: 'Colheita Seja Semente', platform: 'IOS' } },
  });
...
// Perfil
const perfis = await api('GET', `/profiles?filter[name]=${encodeURIComponent('Colheita AppStore')}`);
const antigo = (perfis.json.data || []).find(p => p.attributes.name === 'Colheita AppStore');
if (antigo) await api('DELETE', `/profiles/${antigo.id}`);
const novo = await api('POST', '/profiles', {
  data: {
    type: 'profiles',
    attributes: { name: 'Colheita AppStore', profileType: 'IOS_APP_STORE' },
    relationships: {
...
if (novo.json.data?.attributes?.profileContent) {
  console.log('✓ Perfil "Colheita AppStore" criado');
  console.log('── PERFIL nativo-colheita ──');
...
if ((ja.json.data || []).length) console.log(`✓ Ficha já existe (${ja.json.data[0].id})`);
  ...
  const cria = await api('POST', '/apps', {
    data: {
      type: 'apps',
      attributes: { name: 'Colheita Seja Semente', bundleId: BUNDLE, sku: 'colheita-seja-semente', primaryLocale: 'pt-BR' },
    },
  });
  console.log(`POST /apps → ${cria.status}: ${JSON.stringify(cria.json.errors?.[0] || cria.json.data?.id || cria.json).slice(0, 300)}`);
  if (cria.status >= 300) console.log('⚠ A Apple não deixou criar a ficha pela API — criar no site appstoreconnect.apple.com (Meus apps → + → Novo app, nome "Colheita Seja Semente", bundle com.sejasemente.colheita, SKU colheita-seja-semente).');
}
console.log('\n✓ Fim');
