...
ESTILO dos desenhos de ESTRATIFICAÇÃO/camadas (o aluno pediu assim): igual aos mapas de estratificação de protético — contorno simples do dente em VISTA VESTIBULAR (de frente), fundo branco, as zonas de cada massa pintadas em aquarela suave, e o NOME/CÓDIGO de cada massa escrito ao lado com linha fina apontando a zona. Use os códigos REAIS dos materiais da biblioteca do aluno quando existirem (ex.: os nomes das massas do material dele). NÃO desenhe corte anatômico/transversal, a não ser que o aluno peça explicitamente. Descreva tudo isso no [ILUSTRAR: ...] em inglês, incluindo os códigos como labels.
    ...
    for (const modelo of MODELOS) {
      ...
      if (texto) {
        ...
        if (mIl) {
          ...
          if (podeIlustrar) {
            const promptImg = `Clean, professional dental ceramics educational diagram for a dental technician learning feldspathic porcelain, white background, no watermark. If it depicts layering/stratification: draw in the style of a dental lab layering chart — simple frontal (vestibular/labial) tooth outline with soft watercolor-painted layer zones and small text labels with thin pointer lines naming each ceramic mass code; NOT an anatomical cross-section. ${mIl[1].trim()}`;
            const MODELOS_IMG = ['gemini-2.5-flash-image', 'gemini-3.1-flash-image-preview', 'gemini-3-pro-image-preview'];
            for (const mImg of MODELOS_IMG) {
              try {
                const ri = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${mImg}:generateContent?key=` + chave, {
